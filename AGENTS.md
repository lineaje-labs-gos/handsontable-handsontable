...
- `manual-qa.yml` is a standalone **merge** gate, not a Tests-pipeline stage: the `needs-manual-check` label parks a pending `manual-qa` commit status on the PR until a repo member who is not the author comments `/manual-qa passed` (unlabeled PRs get an instant green). It runs on `pull_request_target` with a write-capable token — **never add a checkout or execution of PR-controlled code to it**; API calls only.
